...
const checkRushHourPeriodValidity = (
  rushHourPeriods: RushHourPeriod[]
): boolean => {
  const rushHourPeriodValidity = rushHourPeriods.map(
    (rushHour) =>
      moment(rushHour.from, 'HH:mm', true).isValid() &&
      moment(rushHour.to, 'HH:mm', true).isValid()
  );
...
export const calculateRushHourSurcharge = ({
  ...
  const areAllRushHourPeriodsValid =
    checkRushHourPeriodValidity(rushHourPeriods);
  // Return early if invalid values are passed to rush hour periods
